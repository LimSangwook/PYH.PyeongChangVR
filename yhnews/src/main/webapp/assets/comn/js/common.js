/* jquery-browser Version 2.3 | http://jquery.thewikies.com/browser/ */
...
;(function ($) {
	...
	***********************************************/
	...
	if(isie10 && !chrome && !firefox && !safari || isie11 && !chrome && !firefox && !safari) $html.addClass('ieModern'); //ie10 �댁긽
	...
	************************************************/
	$(function() {
		...
		$('#nav li.nav-'  + $navActive).addClass('active');
		$('.menu li.nav-'  + $navActive).addClass('active');
		...
		var lastScrollTop = 0;
		var delta = 5;
		var $navCont = $('.wrap-header');
		var $navContHeight = $navCont.outerHeight();

		$(window).scroll(function(event){
			var sc = $(window).scrollTop();
			fixScroll = true;
			//if(sc <= 54) $('.wrap-header').addClass('bg-style01');
		});

		PC2018.init(); //怨듯넻�⑥닔

		if($winW >= 769){ //console.log('769 �댁긽 - ��');
			$body.addClass('w-static');
			setInterval(function() {
				if (fixScroll) {
					scrollFixed(1,'','w-static','w-fixed');
				}
				fixScroll = false;
			}, 250);
		}else{
			setInterval(function() {
				if (fixScroll) {
					scrollFixed(1,'','m-static','m-fixed');
				}
				fixScroll = false;
			}, 250);
		}

		function scrollFixed(startH,H,classStatic,classFixed,classNeed) {
			var $selfTop = $(this).scrollTop();
			var windowTop = $(window).scrollTop();

			if(windowTop> startH) $body.removeClass(classStatic).addClass(classFixed);
			else $body.addClass(classStatic).removeClass(classFixed);

			//console.log($selfTop > $navContHeight);
			if(Math.abs(lastScrollTop - $selfTop) <= delta) return;
			if($selfTop > lastScrollTop && $selfTop > 0){
				//if($selfTop > lastScrollTop && $selfTop > $navContHeight){
				//console.log('scroll down : ',$selfTop);
				$body.removeClass('body-up body-static').addClass('body-down');
			}
			else if($selfTop < startH){
				//console.log('scroll - static : ',$selfTop);
				$body.removeClass('body-down').addClass('body-static');
			}
			else {
				//console.log('scroll up');
				if($selfTop + $(window).height() < $(document).height()) {
					$body.removeClass('body-down body-static').addClass('body-up');
				}
			}

			lastScrollTop = $selfTop;
		}

		if($header.hasClass('header-type02')) $header.addClass('mode02');
		else if($header.hasClass('header-type03')) $header.addClass('mode03');
	...
	var PC2018 = {
		...
		scrollUp:function(){
			$('.btn-scroll-top').on('click vclick', function(){
...
* �ㅻ뜑 �됱긽 蹂�寃�
************************************************/
 function headerModeOpen(mode,hType){
	if($header.hasClass(mode)){
		$header.removeClass(hType).addClass('header-type01');
	}
}

 function headerModeClose(mode,hType){
	if($header.hasClass(mode)){
		$header.removeClass('header-type01').addClass(hType);
	}
}

function modeChangeOpen(){
	if($header.hasClass('mode02')){
		headerModeOpen('mode02','header-type02');
	}else if($header.hasClass('mode03')){
		headerModeOpen('mode03','header-type03');
	}
}

function modeChangeClose(){
	if($header.hasClass('mode02')){
		headerModeClose('mode02','header-type02');
	}else if($header.hasClass('mode03')){
		headerModeClose('mode03','header-type03');
	}
}
...
var menu = {
	...
	start : function() {
		menu.m = $('.all-menu');
		menu.btn = $('.btn-all-menu');
	},

	button : function() {
		menu.btn.on('click vclick',function(){
			//$(this).toggleClass('close');
			menu.active();

			if($('.search-area').hasClass('open')){ //search open
				searchClick.active($('.btn-srch-close'),'.btn-srch-open');
				modeChangeOpen();
	...
	active : function() {
		if(menu.m.hasClass('open')){ //close
			menu.m.removeClass('open').addClass('close');
			modeChangeClose();
			...
			menu.m.removeClass('close').addClass('open');
			modeChangeOpen();
			$header.addClass('fixed');
		...
		if(menu.btn.hasClass('close')) menu.btn.removeClass('close');
		else menu.btn.addClass('close');
	}
}
...
var searchClick = (function(){
	return {
		...
		sWrap : null, s : null,
		start: function(){
			searchClick.sWrap = $('.search-area');
			searchClick.s = $('.search-area .text');
		},
		button : function(){
			$('.btn-srch-open').on("click vclick",function() {
				searchClick.active($(this),'.btn-srch-close');
				if($('.all-menu').hasClass('open')){
					menu.active();
					modeChangeOpen();
					$header.addClass('fixed');
				}
			...
			$('.btn-srch-close').on("click vclick",function() {
				searchClick.active($(this),'.btn-srch-open');
			});

			searchClick.s.on('click keyup',function(){
		...
		active : function(self,btn){
			if(searchClick.sWrap.hasClass('open')){ //close
				searchClick.sWrap.removeClass('open').addClass('close');
				searchClick.s.blur();
				modeChangeClose();
				$header.removeClass('fixed');
				$wrap.removeClass('layer-open');
			}else{ //open
				searchClick.sWrap.removeClass('close').addClass('open');
				searchClick.s.focus();
				modeChangeOpen();
				$header.addClass('fixed');
				$wrap.addClass('layer-open');
			...
			//button
			if(self.hasClass('on')){
				self.removeClass('on').siblings(btn).addClass('on');
			}else {
				self.addClass('on').siblings(btn).removeClass('on');
			}
		},
		search : function(){
			var chk_str = $("#totalSearchForm #srchForm");
			if(chk_str.val()) return true;
			else{
				alert("寃��됲븯�� �⑥뼱瑜� �낅젰�� 二쇱떗�쒖슂.");
				chk_str.focus();
				return false;
			}
		}
...
var selectLang = {
	...
	lang : null, menuLang : null, langBtn : null, langList : null, BIG5: null,
	langTxt : "",

	//�꾩뿭 蹂���
	start : function() {
		selectLang.lang = $('html').attr('lang');
		selectLang.BIG5 = $('#charset').attr('content');
		selectLang.menuLang = $('#langMenu');
		selectLang.langList = $('#langMenu ul');
		selectLang.langBtn = $('#langMenu').find('button');
	},

	//踰꾪듉
	over : function() {
		selectLang.menuLang.not(':animated').on('mouseenter', function(){
			selectLang.menuLang.addClass('active');
			//selectLang.langList.stop().slideDown(150, 'easeInOutCubic');
		}).on('mouseleave', function(){
			selectLang.menuLang.removeClass('active');
			//selectLang.langList.slideUp(150, 'easeInOutCubic');
		});
	},

	click : function() {
		selectLang.langBtn.on('click vclick', function(){
			if(selectLang.menuLang.hasClass('active')){
				selectLang.menuLang.removeClass('active');
				selectLang.langList.slideUp(150, 'easeInOutCubic');
			}else{
				selectLang.menuLang.addClass('active');
				selectLang.langList.slideDown(150, 'easeInOutCubic');
			}
		});

		selectLang.langBtn.bind('keydown', function(e){
			if (e.which == 9) selectLang.langList.slideDown(100);
		});

		selectLang.langList.find('li:last-child a').bind('keydown', function(){
			selectLang.langList.slideUp(100).hide();
		});
	},

	//�몄뼱
	language : function(){
		var a = selectLang.lang;
		var b = selectLang.BIG5;
		var lang = 'Language';
		//var lang = selectLang.langtxt;
		var txt = selectLang.langBtn.find('span');

		if( a == 'ko'){
			//lang = '�쒓뎅��'  //援�Ц
			selectLang.menuLang.find('.ko').hide();
			txt.text(lang);
		}
		else if( a == 'en'){
			//lang = 'English'  //�곷Ц
			selectLang.menuLang.find('.en').hide();
			txt.text(lang);
		}
		else if( a == 'zh'){
			//lang = '訝�뻼嶸�鵝�'  //以묐Ц 媛꾩껜
			selectLang.menuLang.find('.cg').hide();
			txt.text(lang);
		}
		else if( a == 'zh' && b == 'text/html; charset=BIG5'){
			//lang = '訝�뻼濚곦퐪'  //以묐Ц 踰덉껜
			selectLang.menuLang.find('.cb').hide();
			txt.text(lang);
		}
		else if( a == 'ja'){
			//lang = '�ζ쑍沃�'  //�쇰낯��
			selectLang.menuLang.find('.ja').hide();
			txt.text(lang);
		}
		else if( a == 'ar'){
			//lang = '晩邈磨�'  //�꾨엻��
			selectLang.menuLang.find('.ar').hide();
			txt.text(lang);
		}
		else if( a == 'es'){
			//lang = 'Espa챰ol'  //�ㅽ럹�몄뼱
			selectLang.menuLang.find('.es').hide();
			txt.text(lang);
		}
		else if( a == 'fr'){
			//lang = 'Fran챌ais'  //�꾨옉�ㅼ뼱
			selectLang.menuLang.find('.fre').hide();
			txt.text(lang);
		}
	}
}